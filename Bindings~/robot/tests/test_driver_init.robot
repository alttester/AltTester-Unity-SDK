...
Test Init Driver With Incorrect Host
    ${host} =  Get Environment Variable  ALTSERVER_HOST
    ${port} =  Get Environment Variable  ALTSERVER_PORT
    Run Keyword And Expect Error    ConnectionTimeoutError: Failed to connect to AltTester® Server host: somehost port: 13000.    Initialize AltDriver    host=somehost    timeout=1
    [Teardown]    Run Keyword And Ignore Error    Stop AltDriver

Test Init Driver With Incorrect Port
    ${host} =  Get Environment Variable  ALTSERVER_HOST
    ${port} =  Get Environment Variable  ALTSERVER_PORT
    Run Keyword And Expect Error    ConnectionTimeoutError: Failed to connect to AltTester® Server host: 127.0.0.1 port: 12345.    Initialize AltDriver    port=12345    timeout=1
    [Teardown]    Run Keyword And Ignore Error    Stop AltDriver