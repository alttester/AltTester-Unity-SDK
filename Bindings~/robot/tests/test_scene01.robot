...
Test Scene Load Notification Listener
    ...
    ${notification_triggered}=    Set Variable    ${False}
    Add Notification Listener    LOADSCENE    Log Scene Loaded
    Load Scene    Scene 1 AltDriverTestScene
    Sleep    1
    Remove Notification Listener    LOADSCENE
    Should Be True    ${notification_triggered}    Notification callback was not triggered

Test Scene Unload Notification Listener
    [Documentation]    Registers a notification listener for scene unload and verifies callback execution.
    ${notification_triggered}=    Set Variable    ${False}
    Add Notification Listener    UNLOADSCENE    Log Scene Unloaded
    Load Scene    Scene 2 Draggable Panel    ${False}
    Unload Scene    Scene 2 Draggable Panel
    Sleep    1
    Remove Notification Listener    UNLOADSCENE
    Should Be True    ${notification_triggered}    Notification callback was not triggered
...
Log Scene Loaded
    [Arguments]    ${result}
    Set Suite Variable    ${notification_triggered}    ${True}
