    ...
    while (true) {
      ...
      if ((responseMessage.error == null
        ...
        throw new AltRecvallException(
            String.format(
                "Response received does not match command send. Expected %s:%s. Got %s:%s",
                data.getCommandName(),
                data.messageId(),
                responseMessage.commandName,
    ...
    switch (error.type) {
      ...
      case AltErrors.errorInvalidCommand:
        throw new InvalidCommandException(
            "Invalid command exception. You may want to set the Managed Stripping Level to"
                + " `Minimal` from Player Settings -> Other Settings -> Optimization.");
