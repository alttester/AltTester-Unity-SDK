...
public class AltReversePortForwarding {
  ...
  public static void reversePortForwardingAndroid(
      int remotePort, int localPort, String deviceId, String adbPath) {
    adbPath = getAdbPath(adbPath);
    log.debug(
        "Setting up reverse port forward for android remote port "
            + remotePort
            + " local port: "
            + localPort);
    ...
    try {
      ...
      throw new ReversePortForwardingException(
          "An exception occurred while running command: " + command, ex);
    }
  ...
  public static void removeReversePortForwardingAndroid(
      int remotePort, String deviceId, String adbPath) {
    adbPath = getAdbPath(adbPath);
    log.debug(
        "Removing android reverse port forwarding remotePort: "
            + remotePort
            + " deviceId: "
            + deviceId);
