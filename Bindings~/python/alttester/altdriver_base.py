...
class AltDriverBase:
    ...
    def _check_server_version(self):
        ...
        logger.info(
            "Connection established with instrumented Unity app. AltTester(R) Version: {}".format(
                server_version
            )
        )
    ...
    def call_static_method(
        self, type_name, method_name, assembly, parameters=None, type_of_parameters=None
    ):
    ...
    def load_scene(self, scene_name, load_single=True):
        ...
        commands.LoadScene.run(self._connection, scene_name, load_single)
    ...
    def wait_for_current_scene_to_be(self, scene_name, timeout=30, interval=1):
        ...
        return commands.WaitForCurrentSceneToBe.run(
            self._connection, scene_name, timeout, interval
        )
    ...
    def get_player_pref_key(self, key_name, key_type):
        ...
        return commands.GetPlayerPrefKey.run(self._connection, key_name, key_type)
    ...
    def set_player_pref_key(self, key_name, value, key_type):
        ...
        commands.SetPlayerPrefKey.run(self._connection, key_name, value, key_type)
    ...
    def __find_object(self, altby: AltBy, camera_altby: AltBy = AltBy.name(""), enabled=True):
    ...
    def __find_objects(self, altby: AltBy, camera_altby: AltBy = AltBy.name(""), enabled=True):
    ...
    def __find_object_which_contains(
    ...
    def __find_objects_which_contain(
        self, altby: AltBy, camera_altby: AltBy = AltBy.name(""), enabled=True
    ):
    ...
    def __wait_for_object_which_contains(
        self,
        altby: AltBy,
        camera_altby: AltBy = AltBy.name(""),
        timeout=20,
    ...
    def __wait_for_object_to_not_be_present(
        self,
        altby: AltBy,
        camera_altby: AltBy = AltBy.name(""),
        timeout=20,
    ...
    def __get_all_elements(self, camera_altby: AltBy = AltBy.name(""), enabled=True):
        ...
        return self.__find_objects(AltBy.path("//*"), camera_altby, enabled=enabled)
    ...
    def scroll(self, speed_vertical=1, duration=0.1, wait=True, speed_horizontal=1):
        ...
        commands.Scroll.run(
            self._connection, speed_vertical, duration, wait, speed_horizontal
        )
    ...
    def swipe(self, start, end, duration=0.1, wait=True):
        ...
        commands.Swipe.run(self._connection, start, end, duration, wait)
    ...
    def get_application_screensize(self):
        ...
        screen_height = self.call_static_method(
            "UnityEngine.Screen", "get_height", "UnityEngine.CoreModule"
        )
    ...
    def hold_button(self, coordinates, duration=0.1, wait=True):
        ...
        return commands.Swipe.run(
            self._connection, coordinates, coordinates, duration, wait
        )
    ...
    def get_static_property(self, component_name, property_name, assembly, max_depth=2):
        ...
        return commands.GetStaticProperty.run(
            self._connection, component_name, property_name, assembly, max_depth
        )

    def set_static_property(
        self, component_name, property_name, assembly, updated_value
    ):
        ...
        return commands.SetStaticProperty.run(
            self._connection, component_name, property_name, assembly, updated_value
        )
    ...
    def add_notification_listener(
        self, notification_type, notification_callback, overwrite=True
    ):
