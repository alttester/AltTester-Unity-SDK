...
class TestScene03:
    ...
    def drop_image_with_multipoint_swipe(self, object_names, duration, wait):
        ...
        for name in object_names:
            alt_object = self.alt_driver.find_object(AltBy.name(name))
    ...
    def drop_image(self, drag_location_name, drop_location_name, duration, wait):
        drag_location = self.alt_driver.find_object(AltBy.name(drag_location_name))
        drop_location = self.alt_driver.find_object(AltBy.name(drop_location_name))
    ...
    def test_pointer_enter_and_exit(self):
        alt_object = self.alt_driver.find_object(AltBy.name("Drop Image"))
    ...
    def test_begin_move_end_touch(self):
        alt_object1 = self.alt_driver.find_object(AltBy.name("Drag Image1"))
        alt_object2 = self.alt_driver.find_object(AltBy.name("Drop Box1"))
        ...
        imageSource = alt_object1.get_component_property(
            "UnityEngine.UI.Image", "sprite.name", "UnityEngine.UI")
        imageSourceDropZone = self.alt_driver.find_object(AltBy.name("Drop Image")).get_component_property(
            "UnityEngine.UI.Image", "sprite.name", "UnityEngine.UI")
