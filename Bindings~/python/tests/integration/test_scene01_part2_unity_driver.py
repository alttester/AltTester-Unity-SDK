...
class TestScene01Part2WithUnityDriver:
    ...
    def test_find_object_with_camera_id(self):
        alt_button = self.alt_driver.find_object(AltBy.path("//Button"))
        alt_button.tap()
        alt_button.tap()

        camera = self.alt_driver.find_object(AltBy.path("//Camera"))
        alt_object = self.alt_driver.find_object(
            AltBy.component("CapsuleCollider"), AltBy.id(str(camera.id)))
        assert alt_object.name == "Capsule"

        camera2 = self.alt_driver.find_object(AltBy.path("//Main Camera"))
        alt_object2 = self.alt_driver.find_object(
            AltBy.component("CapsuleCollider"), AltBy.id(str(camera2.id)))
    ...
    def test_find_objects_with_camera_id(self):
        ...
        alt_objects = self.alt_driver.find_objects(
            AltBy.name("Plane"), AltBy.id(str(camera.id)))
        assert alt_objects[0].name == "Plane"

        camera2 = self.alt_driver.find_object(AltBy.path("//Main Camera"))
        alt_objects2 = self.alt_driver.find_objects(
            AltBy.name("Plane"), AltBy.id(str(camera2.id)))
    ...
    def test_wait_for_object_not_be_present_with_camera_id(self):
        camera = self.alt_driver.find_object(AltBy.path("//Main Camera"))
        self.alt_driver.wait_for_object_to_not_be_present(
            AltBy.name("ObjectDestroyedIn5Secs"),
            AltBy.id(str(camera.id))
        )
    ...
    def test_wait_for_object_with_text_with_camera_id(self):
        name = "CapsuleInfo"
        text = self.alt_driver.find_object(AltBy.name(name)).get_text()
        camera = self.alt_driver.find_object(AltBy.path("//Main Camera"))

        alt_object = self.alt_driver.wait_for_object(
            AltBy.path("//{}[@text={}]".format(name, text)),
            AltBy.id(str(camera.id)),
            timeout=1
        )
    ...
    def test_find_object_with_tag(self):
        ...
        alt_object = self.alt_driver.find_object(
            AltBy.component("CapsuleCollider"),  AltBy.tag("MainCamera"))
        assert alt_object.name == "Capsule"

        alt_object2 = self.alt_driver.find_object(
            AltBy.component("CapsuleCollider"), AltBy.tag("Untagged"))
        assert alt_object.x != alt_object2.x
        assert alt_object.y != alt_object2.y
    ...
    def test_find_objects_with_tag(self):
        ...
        alt_object = self.alt_driver.find_objects(
            AltBy.name("Plane"), AltBy.tag("MainCamera"))
        assert alt_object[0].name == "Plane"

        alt_object2 = self.alt_driver.find_objects(
            AltBy.name("Plane"), AltBy.tag("Untagged"))
    ...
    def test_wait_for_object_not_be_present_with_tag(self):
        self.alt_driver.wait_for_object_to_not_be_present(
            AltBy.name("ObjectDestroyedIn5Secs"), AltBy.tag("MainCamera"))
    ...
    def test_find_object_by_camera(self):
        button = self.alt_driver.find_object(AltBy.path("//Button"))
        button.tap()
        button.tap()

        alt_object = self.alt_driver.find_object(
            AltBy.component("CapsuleCollider"),
            AltBy.name("Camera")
        )
    ...
    def test_find_objects_by_camera(self):
        ...
        alt_object = self.alt_driver.find_objects(
            AltBy.name("Plane"), AltBy.name("Camera"))
        assert alt_object[0].name == "Plane"

        alt_object2 = self.alt_driver.find_objects(
            AltBy.name("Plane"), AltBy.name("Main Camera"))
        assert alt_object[0].x != alt_object2[0].x
        assert alt_object[0].y != alt_object2[0].y
    ...
    def test_wait_for_object_which_contains_by_camera(self):
        alt_object = self.alt_driver.wait_for_object_which_contains(
            AltBy.name("Canva"), AltBy.name("Main Camera"))
        assert alt_object.name == "Canvas"
    ...
    def test_get_parent(self):
        element = self.alt_driver.find_object(AltBy.name("Canvas/CapsuleInfo"))
        element_parent = element.get_parent()
        assert element_parent.name == "Canvas"

    def test_tap_coordinates(self):
        capsule_element = self.alt_driver.find_object(AltBy.name("Capsule"))
        self.alt_driver.tap(capsule_element.get_screen_position())
        self.alt_driver.wait_for_object(
            AltBy.path("//CapsuleInfo[@text=Capsule was clicked to jump!]"), timeout=1)
    ...
    def test_camera_not_found_exception(self):
        with pytest.raises(exceptions.CameraNotFoundException):
            self.alt_driver.find_object(AltBy.name("Capsule"), AltBy.name("Camera"))

    def test_input_field_events(self):
        input_field = self.alt_driver.find_object(
            AltBy.name("UnityUIInputField")).set_text("example", submit=True)
    ...
    def test_reset_input(self):
        ...
        assert self.alt_driver.find_object(AltBy.name("AltTesterPrefab")).get_component_property(
        ...
        countKeyDown = self.alt_driver.find_object(AltBy.name("AltTesterPrefab")).get_component_property(
