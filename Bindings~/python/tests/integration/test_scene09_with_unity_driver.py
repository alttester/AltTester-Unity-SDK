...
class TestScene09:
    ...
    def test_scroll_element_NIS(self):
        ...
        self.alt_driver.move_mouse(self.alt_driver.find_object(
            AltBy.name("Scroll View")).get_screen_position(), duration=0.3, wait=True)
    ...
    def test_swipe_NIS(self):
        scrollbar = self.alt_driver.find_object(AltBy.name("Handle"))
        scrollbar_position = scrollbar.get_screen_position()
        button = self.alt_driver.find_object(
            AltBy.path("//Scroll View/Viewport/Content/Button (4)"))
        ...
        scrollbar_final = self.alt_driver.find_object(AltBy.name("Handle"))
