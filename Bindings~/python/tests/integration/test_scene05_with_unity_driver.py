...
class TestScene05:
    ...
    def test_movement_cube(self):
        cube = self.alt_driver.find_object(AltBy.name("Player1"))
    ...
    def test_creating_stars(self):
        stars = self.alt_driver.find_objects_which_contain(
            AltBy.name("Star"), AltBy.name("Player2"))
        assert len(stars) == 1

        self.alt_driver.find_objects_which_contain(
            AltBy.name("Player"), AltBy.name("Player2"))
        pressing_point_1 = self.alt_driver.find_object(
            AltBy.name("PressingPoint1"), AltBy.name("Player2"))
        ...
        pressing_point_2 = self.alt_driver.find_object(
            AltBy.name("PressingPoint2"), AltBy.name("Player2"))
        self.alt_driver.move_mouse(
        ...
        stars = self.alt_driver.find_objects_which_contain(AltBy.name("Star"))
        assert len(stars) == 3
    ...
    def test_power_joystick(self):
        ...
        axis_name = self.alt_driver.find_object(AltBy.name("AxisName"))
        axis_value = self.alt_driver.find_object(AltBy.name("AxisValue"))
    ...
    def test_scroll(self):
        player2 = self.alt_driver.find_object(AltBy.name("Player2"))
    ...
    def test_key_down_and_key_up(self):
        ...
        last_key_down = self.alt_driver.find_object(AltBy.name("LastKeyDownValue"))
        last_key_press = self.alt_driver.find_object(
            AltBy.name("LastKeyPressedValue"))

        assert last_key_down.get_text() == "97"
        assert last_key_press.get_text() == "97"

        self.alt_driver.key_up(AltKeyCode.A)
        last_key_up = self.alt_driver.find_object(AltBy.name("LastKeyUpValue"))
