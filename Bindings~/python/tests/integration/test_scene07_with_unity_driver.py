...
class TestScene07A:
    ...
    def test_tap_coordinates_NIS(self):
        ...
        action_info = self.alt_driver.wait_for_object(
            AltBy.path("//ActionText[@text=Capsule was tapped!]"), timeout=1)
    ...
    def test_click_coordinates_NIS(self):
        ...
        action_info = self.alt_driver.wait_for_object(
            AltBy.path("//ActionText[@text=Capsule was clicked!]"),
            timeout=1
        )
    ...
    def test_tilt(self):
        cube = self.alt_driver.find_object(AltBy.name("Cube (1)"))
        ...
        assert initial_position != self.alt_driver.find_object(
            AltBy.name("Cube (1)")).get_world_position()
...
class TestScene07B:
    ...
    def get_sprite_name(self, source_image_name, image_source_drop_zone_name):
        image_source = self.alt_driver.find_object(AltBy.name(source_image_name)).get_component_property(
