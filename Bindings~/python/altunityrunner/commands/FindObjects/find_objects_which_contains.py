...
class FindObjectsWhichContain(CommandReturningAltElements):
    ...
    def __init__(self, socket, request_separator, request_end, by, value, camera_by, camera_path, enabled):
        super(FindObjectsWhichContain, self).__init__(socket, request_separator, request_end)
        self.by = by
        self.value = value
    ...
    def execute(self):
        ...
        if self.enabled:
            data = self.send_command("findObjects", path, By.return_enum_string(self.camera_by), camera_path, "true")
        else:
            data = self.send_command("findObjects", path, By.return_enum_string(self.camera_by), camera_path, "false")
