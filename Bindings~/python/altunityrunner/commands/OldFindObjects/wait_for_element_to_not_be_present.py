...
class WaitForElementToNotBePresent(CommandReturningAltElements):
    def __init__(self, socket,request_separator,request_end,appium_driver,value,camera_name='', timeout=20, interval=0.5,enabled=True):
        super(WaitForElementToNotBePresent, self).__init__(socket,request_separator,request_end,appium_driver)
        self.value=value
    ...
    def execute(self):
        ...
        while (t <= self.timeout):
            try:
                logger.trace('Waiting for element ' + self.value + ' to not be present...')
