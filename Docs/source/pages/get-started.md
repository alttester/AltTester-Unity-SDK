...
.. note::
    ...
    If you are building your instrumented app using the `IL2CPP` Scripting Backend configuration, you also may want to set the Managed Stripping Level to `Minimal` from Player Settings -> Other Settings -> Optimization to avoid raising an exception and impossibility  to connect to the AltTester® Desktop.
