    ...
    public class DriverCommunicationWebSocket : IDriverCommunication
    {
        ...
        public void Connect()
        {
            ...
            if (watch.Elapsed.TotalSeconds > _connectTimeout && !wsClient.IsAlive)
                throw new ConnectionTimeoutException(string.Format("Failed to connect to Alt Tester on host: {0} port: {1}.", _host, _port));

            if (!wsClient.IsAlive)
                throw new ConnectionException(string.Format("Failed to connect to Alt Tester on host: {0} port: {1}.", _host, _port));
