...
class WaitForElementWithText(CommandReturningAltElements):
    def __init__(self, socket,requestSeparator,requestEnd,value,text,camera_name='', timeout=20, interval=0.5,enabled=True):
    ...
    def execute(self):
        ...
        while (t <= self.timeout):
            try:
                ...
                object_text=GetText(self.socket,self.requestSeparator,self.requestEnd,alt_object).execute()
